        ...
        html += `
            <div class="role-card${isCurrentRole ? ' role-card--active' : ''}">
                <div class="role-card__header">
                    <span class="material-symbols-outlined role-card__icon">${info.icon}</span>
                    <h3 class="role-card__title">${role}</h3>
                    ${isCurrentRole ? '<span class="role-card__badge">Rol actual</span>' : ''}
                </div>
...
document.querySelectorAll('.modal__tab').forEach(tab => {
    tab.addEventListener('click', function() {
        ...
        document.querySelectorAll('.modal__panel').forEach(p => p.classList.remove('modal__panel--active'));
        
        this.classList.add('modal__tab--active');
        document.querySelector(`.modal__panel[data-panel="${panelName}"]`).classList.add('modal__panel--active');
