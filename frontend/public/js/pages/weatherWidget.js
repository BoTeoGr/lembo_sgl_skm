...
function showWeatherError() {
	...
	if (weatherCard) {
		weatherCard.innerHTML =
			'<div style="color: #c00; text-align: center; padding: 1em;">No se pudo obtener el clima actual.<br>Verifica tu conexión o intenta más tarde.</div>';
	}
}
...
function updateWeatherCard(data) {
	...
	if (conditionText.toLowerCase().includes("lluvia"))
		iconClass = "fa-cloud-showers-heavy";
	if (conditionText.toLowerCase().includes("nublado")) iconClass = "fa-cloud";
	if (
		conditionText.toLowerCase().includes("soleado") ||
		conditionText.toLowerCase().includes("despejado")
	)
		iconClass = "fa-sun";
	if (conditionText.toLowerCase().includes("nieve")) iconClass = "fa-snowflake";
	if (
		conditionText.toLowerCase().includes("tormenta") ||
		conditionText.toLowerCase().includes("trueno")
	)
		iconClass = "fa-bolt";
	...
	if (
		...
		typeof data.current.air_quality["pm2_5"] !== "undefined"
	) {
		...
		if (pm25 <= 12) {
			airQuality = {
				...
				level: "good",
				description:
					"Calidad del aire satisfactoria, sin riesgos para la salud.",
			};
		} else if (pm25 <= 35) {
			airQuality = {
				text: "Moderada",
				level: "moderate",
				description:
					"Calidad del aire aceptable. Puede afectar a personas sensibles.",
			};
		} else if (pm25 <= 55) {
			airQuality = {
	...
	if (stats[2])
		stats[2].textContent = `${Math.round(data.current.feelslike_c)}°`;
}
