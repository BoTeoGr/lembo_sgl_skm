...
const insumosDisponibles = [
	{
		...
		descripcion:
			"Fertilizante completo con balance de nitrógeno, fósforo y potasio para crecimiento equilibrado",
		icono: "leaf",
		iconoColor: "#22c55e",
		unidad: "kg",
	},
	{
		...
		descripcion:
			"Fertilizante de aplicación foliar con micronutrientes para corregir deficiencias",
		icono: "leaf",
		iconoColor: "#22c55e",
		unidad: "L",
	},
	{
		...
		presentacion: "Líquido",
		descripcion:
			"Insecticida a base de extractos naturales para control de plagas",
		icono: "bolt",
		iconoColor: "#f59e0b",
		unidad: "L",
	},
	...
	},
	{
		...
		presentacion: "Líquido",
		descripcion:
			"Estimulante para el desarrollo de raíces y mejora de absorción de nutrientes",
		icono: "seedling",
		iconoColor: "#10b981",
		unidad: "L",
	},
...
const ciclosDisponibles = [
	...
	},
	{
		...
		duracionDias: 110,
		descripcion:
			"Ciclo de cultivo para tomate Roma en temporada de verano con mayor temperatura",
	...
	sensoresIntegrados.forEach((sensor) => {
      ...
      <div class="sensor__card">
        <div class="sensor__icon">
          <i class="fas fa-${sensor.icono}" style="color: ${
			sensor.iconoColor
		}"></i>
        </div>
        ...
        <div class="sensor__status">
          ...
          <button class="btn btn--danger btn--small delete-integration" data-id="${
						sensor.id
					}">
    ...
    `;
		...
		deleteBtn.addEventListener("click", (e) => {
			...
			if (confirm("¿Está seguro de eliminar esta integración?")) {
				sensoresIntegrados = sensoresIntegrados.filter(
					(s) => s.id !== sensorId
				);
				renderSensoresIntegrados();
				showToast(
	...
	insumosIntegrados.forEach((insumo) => {
      ...
      <div class="insumo__card">
        <div class="insumo__icon">
          <i class="fas fa-${insumo.icono}" style="color: ${
			insumo.iconoColor
		}"></i>
        </div>
        <div class="insumo__info">
          ...
          <div class="insumo__meta">
            ...
            ${
							insumo.unidad
								? `<span class="insumo__unit">${insumo.unidad}</span>`
								: ""
						}
          </div>
        </div>
        <div class="insumo__status">
          <span class="insumo__usage">Último uso: ${
						insumo.ultimoUso || "No usado"
					}</span>
	...
	if (currentIntegrationType === "sensor") {
		if (sensoresIntegrados.length > 0) {
			sensoresIntegrados.forEach((sensor) => {
				...
				div.className = `selection-item ${
					selectedSensor && selectedSensor.id === sensor.id ? "selected" : ""
				}`;
        ...
        `;
				...
				div.addEventListener("click", () => {
					...
					if (selectedItemInfo) {
						...
						selectedItemDescription.textContent = `Ubicación: ${
							sensor.ubicacion
						}, Intervalo: ${formatIntervalo(sensor.intervalo)}`;
					}
				});
		...
		if (insumosIntegrados.length > 0) {
			insumosIntegrados.forEach((insumo) => {
				const div = document.createElement("div");
				div.className = `selection-item ${
					selectedInsumo && selectedInsumo.id === insumo.id ? "selected" : ""
				}`;
        ...
        `;
				...
				div.addEventListener("click", () => {
					...
					if (selectedItemInfo) {
						...
						selectedItemDescription.textContent = `Presentación: ${
							insumo.presentacion
						}, Último uso: ${insumo.ultimoUso || "No usado"}`;
					}
				});
		...
		btnDelete.onclick = () => {
			if (currentIntegrationType === "sensor") {
				if (!selectedSensor) {
					showToast(
						"Error",
						"Por favor seleccione un sensor para eliminar",
						"error"
					);
					return;
				}
				...
				showToast(
					"Integración eliminada",
					`${selectedSensor.nombre} ha sido eliminado correctamente.`,
					"success"
				);
				...
				insumosIntegrados = insumosIntegrados.filter(
					(insumo) => insumo.id !== selectedInsumo.id
				);
				renderInsumosIntegrados(); // Actualizar la lista de insumos integrados
				showToast(
					"Integración eliminada",
					`${selectedInsumo.nombre} ha sido eliminado correctamente.`,
					"success"
				);
			...
			option.addEventListener("change", (e) => {
				...
				if (searchInput) {
					...
					searchInput.placeholder = `Buscar ${
						currentIntegrationType === "sensor" ? "sensor" : "insumo"
					} por nombre, tipo o ${
						currentIntegrationType === "sensor" ? "modelo" : "marca"
					}...`;
				}
		...
		if (!isDeleteMode) {
			if (currentIntegrationType === "sensor") {
				...
				showToast(
					...
					`Se ha agregado ${selectedSensor.nombre} correctamente.`,
					"success"
				);
			} else {
				if (!selectedInsumo) {
				...
				showToast(
					"Integración agregada",
					`Se ha agregado ${selectedInsumo.nombre} correctamente.`,
					"success"
				);
			}
        ...
        `;
              ...
              <div class="detail-grid">
                ...
                <div class="detail-item">
                  ...
                  <span class="detail-value">${getRangoMedicion(
										sensor.tipo
									)}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">Precisión:</span>
                  <span class="detail-value">${getPrecision(
										sensor.tipo,
										sensor.modelo
									)}</span>
                </div>
        ...
        `;
              ...
              <div class="detail-grid">
                ...
                <div class="detail-item">
                  ...
                  <span class="detail-value">${getMetodoAplicacion(
										insumo.tipo
									)}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">Frecuencia recomendada:</span>
                  <span class="detail-value">${getFrecuenciaAplicacion(
										insumo.tipo
									)}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">Personal requerido:</span>
                  <span class="detail-value">${getPersonalRequerido(
										insumo.tipo
									)}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">Precauciones:</span>
                  <span class="detail-value">${getPrecauciones(
										insumo.tipo
									)}</span>
                </div>
                <div class="detail-item descripcion">
...
function showToast(title, message, type = "success") {
	...
	if (type === "success") {
		toastIcon.className =
			"fas fa-check-circle toast__icon toast__icon--success";
	} else {
		toastIcon.className =
			"fas fa-exclamation-circle toast__icon toast__icon--error";
	}
...
function aplicarCiclo(ciclo) {
	...
	showToast(
		"Ciclo actualizado",
		`Se ha aplicado el ciclo "${ciclo.nombre}" correctamente.`,
		"success"
	);
}
...
function renderCycleSelectionList() {
	...
	if (filteredCiclos.length > 0) {
		filteredCiclos.forEach((ciclo) => {
			const div = document.createElement("div");
			div.className = `selection-item ${
				selectedCycle && selectedCycle.id === ciclo.id ? "selected" : ""
			}`;
