import {
	fetchCiclosCultivoFromAPI,
	renderCiclosCultivoTable,
	updateCicloCultivoEstadoAPI,
	ciclosCultivosConfig,
} from "../config/ciclosCultivosConfig.js";
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	function getFilteredCiclos() {
		...
		return allCiclos.filter((c) => {
			...
			const matchesEstado =
				!estadoSel || normalizeEstado(c.estado) === estadoCanon;
	...
	function renderPaginatedTable(list) {
		...
		controlsDiv
			.querySelectorAll(".pagination__button--page")
	...
	function showCicloCultivoDetails(ciclo) {
		...
		document.getElementById("modalCicloCultivoNombre").textContent =
			ciclo.nombre || "";
		document.getElementById("modalCicloCultivoDescripcion").textContent =
			ciclo.descripcion || "";
		document.getElementById("modalCicloCultivoNovedades").textContent =
			ciclo.novedades || "";
		document.getElementById("modalCicloCultivoPeriodoInicio").textContent =
			ciclo.periodoInicio || "";
		document.getElementById("modalCicloCultivoPeriodoFinal").textContent =
			ciclo.periodoFinal || "";
		document.getElementById("modalCicloCultivoEstado").textContent =
			ciclo.estado || "";
		...
		document
			.getElementById("viewCicloCultivoModal")
	...
	if (generateReportBtn && reportModal) {
		generateReportBtn.onclick = (e) => {
			...
			const filename = `reporte_ciclos_cultivos_${new Date()
