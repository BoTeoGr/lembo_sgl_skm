async function fetchSensorsStats() {
	try {
		...
		if (!response.ok)
			throw new Error("Error al obtener estadísticas de sensores");
...
function updateSensorCard(stats) {
	...
	const sensorCard = Array.from(document.querySelectorAll(".card__title")).find(
		(el) => el.textContent.includes("Sensores")
	);
	...
	statusItems.forEach((item) => {
		...
		const labelSpan = Array.from(spans).find(
			(span) => !span.classList.contains("status-list__dot")
		);
	...
	const footerBadge = document.querySelector(
		".footer-actions .badge:last-child"
	);
	if (footerBadge) {
