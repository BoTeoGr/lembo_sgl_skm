...
function determineCategory(tipo) {
	...
	if (
		tipoLower.includes("temperatura") ||
		tipoLower.includes("humedad") ||
		tipoLower.includes("luz") ||
		tipoLower.includes("presion") ||
		tipoLower.includes("viento")
	) {
		return "Ambiental";
	} else if (
		tipoLower.includes("suelo") ||
		tipoLower.includes("ph") ||
		tipoLower.includes("nutriente")
	) {
		return "Suelo";
	} else if (
		tipoLower.includes("riego") ||
		tipoLower.includes("agua") ||
		tipoLower.includes("flujo")
	) {
		return "Riego";
	} else if (
		tipoLower.includes("calidad") ||
		tipoLower.includes("co2") ||
...
async function fetchSensorsFromAPI() {
	try {
		...
		console.warn(
			"Fallo la carga desde la API, usando datos locales:",
			e.message
		);
		return [];
	}
...
class Sensors {
	...
	initializeEventListeners() {
		...
		document
			...
			?.addEventListener("change", () => this.renderReportPreview());
		document
		...
		document.querySelector(".table__body").addEventListener("click", (e) => {
			...
			if (btn.classList.contains("table__action-button--view")) {
				...
				const sensor = this.filteredData.find(
		...
		document.getElementById("closeViewSensorModal").onclick = () => {
			document
				.getElementById("viewSensorModal")
	...
	filterData() {
		...
		const typeFilter = document.querySelector(
			'.filters__select[placeholder="Tipo de Sensor"]'
		).value;
		...
		const normalizeStatus = (val) => {
			const v = String(val || "")
				.trim()
				.toLowerCase();
			if (["activo", "habilitado", "true", "1", "enabled"].includes(v))
				return "activo";
			if (["inactivo", "deshabilitado", "false", "0", "disabled"].includes(v))
				return "inactivo";
			return v;
		};
		...
		this.filteredData = this.filteredData.filter((sensor) => {
			...
			const matchesSearch =
				!searchTerm || searchFields.some((field) => field.includes(searchTerm));

			const matchesStatus =
				!statusFilter ||
				normalizeStatus(sensor.estado) === normalizedSelectedStatus;

			const sensorType = String(sensor.tipo || sensor.tipo_sensor || "")
				.toLowerCase()
				.trim();
	...
	updatePagination() {
		...
		document.querySelector(".pagination__current-page").textContent =
			(this.currentPage - 1) * this.itemsPerPage + 1;
		document.querySelector(".pagination__items-per-page").textContent =
	...
	generateReport() {
		...
		const includeMaintenance =
			document.getElementById("includeMaintenance")?.checked;
		...
		if (!includeInactive)
			dataset = dataset.filter(
				(s) => String(s.estado).toLowerCase() !== "deshabilitado"
			);
