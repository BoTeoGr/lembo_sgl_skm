...
function setupProductionStatusCard() {
	...
	const mainValue = document.querySelector(
		".production-status__main .stat__value"
	);
	const trendValue = document.querySelector(
		".production-status__main .stat__trend-value"
	);
	const detailsEnProceso = document.querySelector(
		".production-status__details .status-item:nth-child(1) .status-item__value"
	);
	const detailsPorIniciar = document.querySelector(
		".production-status__details .status-item:nth-child(2) .status-item__value"
	);
	const detailsCompletadas = document.querySelector(
		".production-status__details .status-item:nth-child(3) .status-item__value"
	);
	...
	window.actualizarProductionStatusCard = function (producciones) {
		...
		producciones.forEach((p) => {
			...
			if (p.estado === "habilitado" && p.progreso > 0 && p.progreso < 100)
				enProceso++;
			if (p.estado === "habilitado" && p.progreso === 0) porIniciar++;
			if (
				p.progreso === 100 &&
				new Date(p.fecha_fin).getFullYear() === añoActual
			)
				completadas++;
			// Para tendencia: contar activas del mes pasado
			if (
				p.estado === "habilitado" &&
				new Date(p.fecha_de_inicio).getMonth() === mesActual - 1
			)
				activasMesPasado++;
		});
			...
			tooltip: {
				...
				callbacks: {
					label: (context) => {
						...
						document.addEventListener("DOMContentLoaded", () => {
							if (
								!window.location.pathname.endsWith("actualizar-produccion.html")
							)
								return;
							...
							const insumosContainer =
								document.getElementById("selectedSupplies");
							const sensoresContainer =
								document.getElementById("selectedSensors");
							...
							fetch(`http://localhost:5000/producciones/${productionId}`)
								...
								.then((data) => {
									...
									if (startDateInput)
										startDateInput.value = data.fecha_de_inicio
											? data.fecha_de_inicio.slice(0, 10)
											: "";
									if (endDateInput)
										endDateInput.value = data.fecha_fin
											? data.fecha_fin.slice(0, 10)
											: "";
									if (investmentInput)
										investmentInput.value = data.inversion || "";
									...
									// Llenar selects dinámicos (cultivo, ciclo, responsable)
									Promise.all([
										fetch("http://localhost:5000/cultivos").then((res) =>
											res.json()
										),
										fetch("http://localhost:5000/ciclos-cultivo").then((res) =>
											res.json()
										),
										fetch("http://localhost:5000/usuarios").then((res) =>
											res.json()
										),
										...
										cropSelect.innerHTML =
											...
											cultivos
												.map(
													(c) =>
														`<option value="${c.id}"${
															String(c.id) === String(data.cultivo_id)
																? " selected"
																: ""
														}>${c.nombre}</option>`
												)
										...
										cycleSelect.innerHTML =
											...
											ciclos
												.map(
													(c) =>
														`<option value="${c.id}"${
															String(c.id) === String(data.ciclo_id)
																? " selected"
																: ""
														}>${c.nombre}</option>`
												)
										...
										responsibleSelect.innerHTML =
											...
											listaUsuarios
												.map(
													(u) =>
														`<option value="${u.id}"${
															String(u.id) === String(data.usuario_id)
																? " selected"
																: ""
														}>${u.nombre}</option>`
												)
									...
									if (Array.isArray(data.insumos) && data.insumos.length > 0) {
										...
										data.insumos.forEach((insumo) => {
											...
											item.innerHTML = `
										<span>${insumo.nombre} (Cantidad: <input type="number" min="0" value="${
												insumo.cantidad || ""
											}" data-insumo-id="${
												insumo.id
											}" class="insumo-cantidad-input" style="width:60px">)</span>
										<button type="button" class="remove-insumo-btn" data-insumo-id="${
											insumo.id
										}">Quitar</button>
									`;
										...
										insumosContainer.innerHTML =
											'<span style="color:#888">No hay insumos seleccionados.</span>';
									}
									...
									if (
										...
										<button type="button" class="remove-sensor-btn" data-sensor-id="${
											sensor.id
										}">Quitar</button>
									`;
										...
										sensoresContainer.innerHTML =
											'<span style="color:#888">No hay sensores seleccionados.</span>';
									}
								});
							...
							form.addEventListener("submit", function (e) {
								...
								if (
									!form.productionName.value.trim() ||
									!form.productionType.value ||
								...
								const insumos = Array.from(
									insumosContainer.querySelectorAll(".insumo-cantidad-input")
								).map((input) => ({
									id: input.getAttribute("data-insumo-id"),
									cantidad: input.value,
								}));
								const sensores = Array.from(
									sensoresContainer.querySelectorAll(".sensor-item")
								).map((item) => {
								...
								const updatedData = {
									...
									fecha_de_inicio: startDateInput
										? startDateInput.value
										: undefined,
									fecha_fin: endDateInput ? endDateInput.value : undefined,
									inversion: investmentInput
...
function setupFilters() {
	...
	const stateSelect = document.querySelector(
		'.filters__select[placeholder="Estado"]'
	);
	...
	function filterTable() {
		...
		const selectedState =
			selectedStateRaw === "Activo"
				? "habilitado"
				: selectedStateRaw === "Inactivo"
				? "deshabilitado"
				: "";
		// Ciclo removido
		...
		getRows().forEach((row) => {
			...
			const stateText = (row.querySelector(".badge--status")?.textContent || "")
				.trim()
				.toLowerCase();
			// Ciclo removido
			...
			// Aplicar filtros
			const matchesSearch =
				!searchTerm || id.includes(searchTerm) || name.includes(searchTerm);
...
function setupPagination() {
	...
	const getEligibleRows = () =>
		getAllRows().filter((r) => r.dataset.filtered !== "true");
	...
	const getControlsContainer = () =>
		document.querySelector(".pagination__controls");
	...
	function refreshPaginationUI() {
		...
		const itemsPerPageEl = document.querySelector(
	...
	function showPage(page) {
		...
		Array.from(
			document.querySelectorAll(".pagination__controls .pagination__button")
		)
			.filter(
				(btn) =>
					!btn.classList.contains("pagination__button--prev") &&
			...
			.forEach((btn) => {
				btn.classList.toggle(
					...
					btn.textContent === String(currentPage)
				);
			});
		if (prevBtn) prevBtn.disabled = currentPage === 1;
		if (nextBtn) nextBtn.disabled = currentPage === totalPages;
...
function setupSeleccionMultiple() {
	...
	function actualizarSeleccion() {
		...
		const allCheckboxes = Array.from(
			document.querySelectorAll(".table__checkbox")
		);
		...
		const allCheckedVisible =
			eligibleVisibleCheckboxes.length > 0 &&
			eligibleVisibleCheckboxes.every((cb) => cb.checked);
		const someCheckedVisible = eligibleVisibleCheckboxes.some(
			(cb) => cb.checked
		);
		if (checkboxHeader) {
		...
		const allCheckedAll =
			eligibleAllCheckboxes.length > 0 &&
			eligibleAllCheckboxes.every((cb) => cb.checked);
	...
	if (checkboxHeader) {
		checkboxHeader.addEventListener("change", function () {
			const eligibleCheckboxes = Array.from(
				document.querySelectorAll(".table__row")
			)
				.filter(
					(r) => r.dataset.filtered !== "true" && r.style.display !== "none"
				)
...
async function habilitarProduccionesMultiples(ids) {
	try {
		...
		const response = await fetch(
			"http://localhost:5000/producciones/estados/habilitado",
			{
				method: "PUT",
				headers: {
		...
		mostrarMensaje(
			`${ids.length} producciones habilitadas correctamente`,
			"success"
		);
	} catch (error) {
...
async function deshabilitarProduccionesMultiples(ids) {
	try {
		...
		const response = await fetch(
			"http://localhost:5000/producciones/deshabilitar",
			{
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
		...
		// Mostrar mensaje de éxito
		mostrarMensaje(
			`${ids.length} producciones deshabilitadas correctamente`,
			"success"
		);
	} catch (error) {
...
function mostrarMensaje(mensaje, tipo = "info") {
	...
	if (toastTitle)
		toastTitle.textContent = tipo === "success" ? "Éxito" : "Información";
...
function updatePaginationAfterFilter() {
	...
	const totalPagesEl = document.querySelector(".pagination__total-pages");
	if (totalPagesEl)
		totalPagesEl.textContent = String(
...
function setupReportGeneration() {
	...
	function collectReportData() {
		...
		const selectedCols = Array.from(
			document.querySelectorAll(".col-check:checked")
	...
	function renderPreview() {
		...
		prev.innerHTML =
			count > 0
				? `Se exportarán <strong>${count}</strong> producciones en <strong>${
						format || "CSV"
				  }</strong> con <strong>${selectedCols.length}</strong> columnas.`
	...
	document
		...
		?.addEventListener("change", renderPreview);
	Array.from(document.querySelectorAll(".col-check")).forEach((el) =>
	...
	document.getElementById("reportForm")?.addEventListener("submit", (e) => {
		...
		if (format === "json") {
			const blob = new Blob(
				[JSON.stringify({ columnas: cols, datos: data }, null, 2)],
				{ type: "application/json;charset=utf-8" }
			);
			const url = URL.createObjectURL(blob);
			const a = document.createElement("a");
			a.href = url;
			a.download = `reporte_producciones_${fecha}.json`;
		...
		if (format === "csv") {
			...
			const a = document.createElement("a");
			a.href = url;
			a.download = `reporte_producciones_${fecha}.csv`;
			document.body.appendChild(a);
			a.click();
			document.body.removeChild(a);
			URL.revokeObjectURL(url);
		...
		if (format === "xlsx" || format === "excel") {
			if (window.ReportGenerator) {
				const columns = cols.map((c) => ({
					header: c[0].toUpperCase() + c.slice(1),
					key: c,
				}));
				window.ReportGenerator.generateReport({
					...
					data,
					format: "excel",
					filename: `reporte_producciones_${fecha}`,
				});
				reportModal.style.display = "none";
			} else {
	...
	try {
		...
		const produccionesConProgreso = producciones.map((produccion) => {
			...
			if (produccion.fecha_de_inicio && produccion.fecha_fin) {
				...
				const tiempoTranscurrido =
					new Date(fechaActual).getTime() - fechaInicio.getTime();
				...
				if (tiempoTotal <= 0) {
					...
					progreso = Math.min(
						100,
						Math.round((tiempoTranscurrido / tiempoTotal) * 100)
					);
				}
			}
		...
		try {
			...
			if (summaryCard) {
				...
				const labels = summaryCard.querySelectorAll(
					".stat-card__details .stat-card__label"
				);
				...
				if (totalEl)
					totalEl.textContent = String(produccionesConProgreso.length);
				// Animar icono
			...
			// Totales financieros
			const totalInversion = produccionesConProgreso.reduce(
				(sum, p) => sum + toNumber(p.inversion),
				0
			);
			const totalMeta = produccionesConProgreso.reduce(
				(sum, p) => sum + toNumber(p.meta_ganancia),
				0
			);
			const roiPct =
				totalInversion > 0
					? Math.round(((totalMeta - totalInversion) / totalInversion) * 100)
					: 0;

			// Actualizar tarjeta "Inversión Total"
			const inversionCard = document.querySelector(
			...
			const metaCard = document.querySelector(
		...
		const totalProduccionesElement =
			document.querySelector(".stat-card__value");
	...
	producciones.forEach((produccion) => {
		...
		<td class="table__cell">
		  <span class="badge badge--status ${
				produccion.estado === "habilitado" ? "badge--active" : "badge--inactive"
			}">
			<span class="material-symbols-outlined">${
				produccion.estado === "habilitado" ? "check_circle" : "cancel"
			}</span>
			${produccion.estado}
		  </span>
		</td>
		<td class="table__cell table__cell--actions">
		  <button class="table__action-button table__action-button--view" data-id="${
				produccion.id
			}">
          ...
          <a href="../views/actualizar-produccion.html?id=${
						produccion.id
					}" class="table__action-button-wrapper">
            <button class="table__action-button table__action-button--edit"><span class="material-symbols-outlined">edit</span></button>
          </a>
        
		  
		  <button class="table__action-button table__action-button--disable" data-id="${
				produccion.id
			}">
	  ...
	  `;
		...
		viewBtn.addEventListener("click", () =>
			verDetallesProduccion(produccion.id)
		);
		disableBtn.addEventListener("click", () =>
			cambiarEstadoProduccion(produccion.id, produccion.estado)
		);
...
async function verDetallesProduccion(id) {
	try {
		...
		if (!response.ok) {
			throw new Error(
				produccion.error || "Error al cargar detalles de la producción"
			);
		}
		...
		const actualizarTexto = (elementId, texto) => {
			...
			if (elemento) {
				...
				console.warn(
					`Elemento con ID "${elementId}" no encontrado en el modal.`
				);
			}
		};
		...
		actualizarTexto(
			"cultivoInversion",
			`$${parseFloat(produccion.inversion || 0).toFixed(2)}`
		);
		actualizarTexto(
			"cultivoMetaGanancia",
			`$${parseFloat(produccion.meta_ganancia || 0).toFixed(2)}`
		);

		// Calcular ROI
		const roi =
			produccion.meta_ganancia && produccion.inversion
				? Math.round(
						((parseFloat(produccion.meta_ganancia) -
							parseFloat(produccion.inversion)) /
							parseFloat(produccion.inversion)) *
							100
				  )
				: 0;
		...
		if (diasRestantesEl) {
			...
			if (produccion.fecha_fin) {
				...
				diasRestantes =
					(isNaN(diff) ? "-" : diff < 0 ? "0" : String(diff)) + " días";
			}
		...
		if (estadoElement) {
			...
			estadoElement.className = `badge badge--status ${
				produccion.estado === "habilitado" ? "badge--active" : "badge--inactive"
			}`;

			const iconElement = estadoElement.querySelector(
		...
		if (listaInsumos) {
			...
			listaInsumos.innerHTML =
				'<li class="insumo-item insumo-item--loading">Cargando insumos...</li>';

			// Mostrar insumos seleccionados
			if (produccion.insumos && produccion.insumos.length > 0) {
				console.log(
					"Datos de insumos recibidos:",
					JSON.stringify(produccion.insumos, null, 2)
				);
				...
				const promesasInsumos = produccion.insumos.map((insumo) => {
					...
					console.log(
						"Solicitando datos completos para insumo ID:",
						insumo.id || insumo.insumo_id
					);
					// Si no, hacemos una petición para obtener los datos completos del insumo
					const token = localStorage.getItem("token");
					return fetch(
						`http://localhost:5000/insumos/${insumo.id || insumo.insumo_id}`,
						{
							headers: {
								Authorization: `Bearer ${token}`,
							},
						...
						.then((datosInsumo) => {
							console.log(
								"Respuesta de la API para insumo:",
								JSON.stringify(datosInsumo, null, 2)
							);
							const insumoData = datosInsumo.insumo || datosInsumo;
							console.log("Datos procesados del insumo:", insumoData);
							const tipo =
								insumoData.tipo || insumoData.tipo_insumo || "No especificado";
				...
				Promise.all(promesasInsumos)
					.then((insumosCompletos) => {
						...
						insumosCompletos.forEach((insumo) => {
							...
							const nombre =
								insumo.nombre || insumo.nombre_insumo || "Sin nombre";
							...
							const valor = parseFloat(
								insumo.valor_total || insumo.valor_unitario || 0
							).toFixed(2);
							const cantidad =
								insumo.cantidad ||
								insumo.cantidad_usar ||
								...
								0;
							const unidad =
								insumo.unidad_medida || insumo.unidad || "unidades";
				...
				<div class="insumo-item__details">
				  ...
				  <div class="insumo-detail">
					<span class="insumo-label">Valor unitario: $${parseFloat(
			  ...
			  `;
					...
					.catch((error) => {
						...
						listaInsumos.innerHTML =
							'<li class="insumo-item insumo-item--error">Error al cargar los insumos. Intente nuevamente.</li>';
					});
			} else {
				const listItem = document.createElement("li");
				listItem.className = "insumo-item insumo-item--empty";
				listItem.textContent =
					"No se han registrado insumos para esta producción.";
				listaInsumos.appendChild(listItem);
			}
		}
		...
		if (listaSensores) {
			...
			console.log(
				...
				JSON.stringify(produccion.sensores, null, 2)
			);
			if (produccion.sensores && produccion.sensores.length > 0) {
				...
				const promesasSensores = produccion.sensores.map((sensor) => {
					...
					if (sensor.id && sensor.nombre_sensor && sensor.tipo_sensor) {
						return Promise.resolve({
							...sensor,
							nombre:
								sensor.nombre_sensor || sensor.nombre || "Sensor sin nombre",
					...
					const token = localStorage.getItem("token");
					return fetch(
						`http://localhost:5000/sensor/${sensor.id || sensor.sensor_id}`,
						{
							headers: {
								Authorization: `Bearer ${token}`,
							},
						...
						.catch((error) => {
							...
							return {
								...sensor,
								nombre:
									sensor.nombre_sensor || sensor.nombre || "Error al cargar",
								tipo: "Error",
								unidad: "N/A",
				...
				Promise.all(promesasSensores)
					.then(async (sensoresCompletos) => {
						// Limpiar el mensaje de carga
						listaSensores.innerHTML = "";
						...
						const fetchSensorDetails = async (sensorId) => {
							try {
								const token = localStorage.getItem("token");
								const response = await fetch(
									`http://localhost:5000/sensor/${sensorId}`,
									{
										headers: {
											Authorization: `Bearer ${token}`,
										},
				  ...
				  <div class="sensor-item__header">
					<span class="sensor-item__name">${
						sensor.nombre_sensor || sensor.nombre || "Cargando..."
					}</span>
				  </div>
				`;
								...
								try {
									...
									const sensorCompleto = await fetchSensorDetails(
									...
									const tipoSensor =
										datosSensor.tipo_sensor || "No especificado";
									const unidadMedida =
										datosSensor.unidad_medida || "No especificada";
									const tiempoEscaneo =
										datosSensor.tiempo_escaneo || "No especificado";

									// Actualizar el elemento con los detalles completos
									listItem.innerHTML = `
					<div class="sensor-item__header">
					  <span class="sensor-item__name">${
							datosSensor.nombre_sensor ||
							datosSensor.nombre ||
							"Sensor sin nombre"
						}</span>
					</div>
				  ...
				  `;
					...
					.catch((error) => {
						...
						listaSensores.innerHTML =
							'<li class="sensor-item sensor-item--error">Error al cargar los sensores. Intente nuevamente.</li>';
					});
			} else {
				const listItem = document.createElement("li");
				listItem.className = "sensor-item sensor-item--empty";
				listItem.textContent =
					"No se han registrado sensores para esta producción.";
				listaSensores.appendChild(listItem);
			}
		}
...
async function cambiarEstadoProduccion(id, estadoActual) {
	try {
		const response = await fetch(
			`http://localhost:5000/producciones/${id}/estado`,
			{
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
		...
		mostrarError(
			"Error al cambiar el estado de la producción: " + error.message
		);
	}
}
	  ...
	  `;
		...
		fetch(`http://localhost:5000/producciones/sensor/${sensorId}`)
			...
			.then((data) => {
				...
				const produccionesListElement =
					listItem.querySelector(".list-group__items");
	...
	insumos.forEach((insumo) => {
		...
		fetch(`http://localhost:5000/producciones/insumo/${insumo.id}`)
			...
			<div class="list-group__header">
			  ...
			  <span class="badge badge--green">Stock: ${insumo.cantidad} ${
