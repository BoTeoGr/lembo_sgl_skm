...
const ReportGenerator = {
    generateReport: function({ columns, data, format = 'excel', filename = 'reporte' }) {
        if (format === 'csv') {
            ...
            this.generateHTMLPdf({ title: filename.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()), columns, data, filename });
        } else {
            alert('Formato no soportado');
        }
    },
    generateHTMLPdf: function({ title = 'Reporte', columns = [], data = [], filename = 'reporte' }) {
        try {
            const html = `
<!DOCTYPE html>
<html lang="es">
...
<body>
    <div class="header-info">
        ...
        <div><strong>Total de registros:</strong> ${Array.isArray(data) ? data.length : 0}</div>
    </div>
    <h1>${title}</h1>
    ...
    <div class="footer">Generado por SGAL · ${new Date().toLocaleString('es-ES')}</div>
    ...
    _toExcelXML: function(columns, data, sheetName = 'Reporte') {
        ...
        const cleanSheetName = String(sheetName).replace(/[\\\/\?\*\[\]]/g, '').slice(0, 31) || 'Reporte';
...
<Styles>
  ...
  <Style ss:ID="sNumber">
    <NumberFormat ss:Format="Standard"/>
        ...
        const workbookOpen = `<?xml version="1.0"?>\n` +
`<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 xmlns:html="http://www.w3.org/TR/REC-html40">\n${styles}\n`;
        ...
        const worksheetOpen = `<Worksheet ss:Name="${this._escapeXml(cleanSheetName)}">\n<Table>\n${colDefs}`;

        const headerRow = `<Row ss:AutoFitHeight="1">${columns.map(col => `<Cell ss:StyleID="sHeader"><Data ss:Type="String">${this._escapeXml(col.header)}</Data></Cell>`).join('')}</Row>`;

        const rows = Array.isArray(data) ? data : [];
        const isNumber = (v) => typeof v === 'number' || (!!v && !isNaN(v) && /^-?\d+(\.\d+)?$/.test(String(v).trim()));
        const dataRows = rows.map((row, idx) => {
            const style = (idx % 2 === 0) ? 'sRow' : 'sAlt';
            const cells = columns.map(col => {
                const val = row[col.key];
                if (isNumber(val)) {
                    return `<Cell ss:StyleID="${style}"><Data ss:Type="Number">${String(val).replace(/,/g,'')}</Data></Cell>`;
                }
                return `<Cell ss:StyleID="${style}"><Data ss:Type="String">${this._escapeXml(val ?? '')}</Data></Cell>`;
            }).join('');
...
<WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
  ...
  <Print>
    ...
    <PaperSizeIndex>9</PaperSizeIndex>
        ...
        return workbookOpen + worksheetOpen + headerRow + dataRows + tableClose + worksheetClose + workbookClose;
    },
