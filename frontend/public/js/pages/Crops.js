		...
		return cultivos.map((cultivo) => {
			...
			if (typeof status === "string") {
				...
				if (["habilitado", "activo", "enabled"].includes(estadoLower)) {
					...
					["deshabilitado", "inhabilitado", "inactivo", "disabled"].includes(
						estadoLower
					)
				) {
					status = "Inhabilitado";
...
class Crops {
	constructor() {
		try {
			this.currentPage = 1;
			this.itemsPerPage =
				(cropsConfig && cropsConfig.table && cropsConfig.table.itemsPerPage) ||
				10;
			this.originalData = [];
			this.filteredData = [];
		...
		try {
			...
			if (!searchTerm.trim()) {
				...
				this.filteredData = this.originalData.filter((crop) => {
					...
					return (
						(crop.id &&
							crop.id.toString().toLowerCase().includes(searchTerm)) ||
						...
						(crop.status &&
							crop.status.toString().toLowerCase().includes(searchTerm)) ||
						(crop.description &&
		...
		setTimeout(() => {
			...
			if (generateReportBtn) {
				generateReportBtn.addEventListener("click", (e) => {
					...
					const includeSensors =
						document.getElementById("includeSensors")?.checked;
					const includeSupplies =
						document.getElementById("includeSupplies")?.checked;
					...
					const filename = `reporte_cultivos_${new Date()
			...
			const renderPreview = () => {
				try {
					...
					const includeInactive =
						document.getElementById("includeInactive")?.checked;
					const crops = Array.isArray(this.filteredData)
						? this.filteredData
						: [];
					const filtered = includeInactive
						? crops
						: crops.filter((c) => c.status === "Activo");
	...
	renderTable() {
		...
		currentPageData.forEach((crop) => {
			...
			row.innerHTML = `
				...
				this.selectedCrops && this.selectedCrops.has(crop.id) ? "checked" : ""
			} />
                </td>
                ...
                <td class="table__cell table__cell--actions">
                    ...
                    ${
											!this.isVisitante
												? `
                    <button class="table__action-button table__action-button--edit" onclick="window.location.href='../views/actualizar-cultivo.html?id=${
											crop.id
										}'">
                        <span class="material-symbols-outlined">edit</span>
                    </button>
                    <button class="table__action-button table__action-button--toggle-status ${
											crop.status === "Activo"
												? "table__action-button--disable"
												: "table__action-button--enable"
										}" data-id="${crop.id}" title="${
														crop.status === "Activo"
															? "Deshabilitar"
															: "Habilitar"
												  }">
			...
			const toggleStatusBtn = row.querySelector(
				".table__action-button--toggle-status"
			);
			if (toggleStatusBtn) {
				toggleStatusBtn.addEventListener("click", async () => {
					const nuevoEstado =
						crop.status === "Activo" ? "Inhabilitado" : "Activo";
	...
	updateUIForVisitante() {
		...
		const actionButtons = document.querySelectorAll(
			".button--add, .button--delete, .button--edit, .button--enable, .button--disable, .button--report, .button--filter, .button--create"
		);
		...
		const checkboxes = document.querySelectorAll(
			".table__checkbox, .table__checkbox-header"
		);
		checkboxes.forEach((checkbox) => {
	...
	showCropModal(crop) {
		...
		document.getElementById("modalCropLocation").textContent =
			crop.location || "";
		document.getElementById("modalCropStatus").textContent = crop.status || "";
		document.getElementById("modalCropDescription").textContent =
			crop.description || crop.descripcion || "-";
		document.getElementById("modalCropCreated").textContent =
			crop.createdAt || crop.fecha_creacion || crop.fechaCreacion || "-";
		// Muestra el modal
