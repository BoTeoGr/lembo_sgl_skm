...
export default function injectProfileModal() {
	...
	const fillFields = (src = {}) => {
		const userRole =
			src.userRol ?? src.rol ?? localStorage.getItem("userRol") ?? "";
		const isAdmin = isAdminUser(userRole);

		const profileFields = {
			profileName:
				src.userName ?? src.nombre ?? localStorage.getItem("userName") ?? "",
			profileEmail:
				src.userEmail ??
				src.correo ??
				src.email ??
			...
			profileTipoDoc:
				...
				src.tipoDoc ??
				src.tipoDocumento ??
				localStorage.getItem("userTipoDoc") ??
				"",
			profileNumDoc:
				src.userNumDoc ??
				...
				src.numDoc ??
				src.documento ??
				localStorage.getItem("userNumDoc") ??
				"",
		};
		...
		if (emailInput) {
			...
			if (!isAdmin) {
				...
				emailInput.title =
					"Solo los administradores pueden modificar el correo";
	...
	function setViewMode(isViewMode) {
		...
		if (isViewMode) {
			...
			const tipoDocShortValue = (
				profileTipoDoc?.getAttribute("data-value") ||
				profileTipoDoc?.textContent ||
				""
			)
				.trim()
				.toLowerCase();
			...
			const tipoDocFullValue =
				docTypeMap[tipoDocShortValue] || tipoDocShortValue;
			...
			for (let i = 0; i < tipoDocSelect.options.length; i++) {
				...
				if (
					option.value.toLowerCase() === tipoDocShortValue ||
			...
			document.getElementById("editNumDoc").value =
				document.getElementById("profileNumDoc").textContent || "";

			// Focus on the first field
			setTimeout(() => {
				const firstInput = document.querySelector(
					"#profileEditForm .form__input, #profileEditForm .form__select"
				);
	...
	document
		...
		.addEventListener("click", async () => {
			...
			if (!userId || !token) {
				showToast(
					"Error",
					"No se pudo verificar su sesión. Por favor, inicie sesión nuevamente.",
					"error"
				);
				return;
			}
			...
			const tipoDocShort =
				reverseDocTypeMap[tipoDocFull.toLowerCase()] || tipoDocFull;
			...
			const currentEmail = document
				...
				.textContent.trim();
			const currentRol = document
				.getElementById("profileRol")
				.textContent.trim();
			...
			try {
				...
				const response = await fetch(
					`http://localhost:5000/usuarios/${userId}`,
					{
						method: "PUT",
						headers: {
				...
				const wasPasswordChanged =
					newPasswordField && newPasswordField.value.trim() !== "";
