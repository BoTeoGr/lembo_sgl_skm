...
document.addEventListener("DOMContentLoaded", function () {
	...
	document.addEventListener("click", function (event) {
		...
		if (
			!sensorSelect.contains(event.target) &&
		...
		if (
			!insumoSelect.contains(event.target) &&
	...
	sensorSelect.addEventListener("change", function () {
		...
		const sensorHelper = this.closest(".form-field").querySelector(
			".form-field__helper"
		);

		// Verificar si el sensor ya está en el grid
		const existingSensor = sensorsGrid.querySelector(
			`.sensor-card[data-id="${sensorId}"]`
		);
		...
		if (existingSensor) {
			...
			const newCard = sensorsGrid.querySelector(
				`.sensor-card[data-id="${sensorId}"]`
			);
			newCard
				.querySelector(".sensor-card__remove")
	...
	insumoSelect.addEventListener("change", function () {
		...
		const insumoHelper = this.closest(".form-field").querySelector(
			".form-field__helper"
		);

		// Verificar si el insumo ya está en el grid
		const existingInsumo = insumosGrid.querySelector(
			`.insumo-card[data-id="${insumoId}"]`
		);
		...
		if (existingInsumo) {
			...
			// Crear la tarjeta HTML
			const cardHtml = `
                <div class="insumo-card insumo-card--${
									insumoInfo.color
								}" data-id="${insumoId}">
                    <div class="insumo-card__header">
                        ...
                        <div class="insumo-card__price">
                            ${insumoInfo.precio}
                            <span class="insumo-card__price-unit">/${
															insumoInfo.unidad
														}</span>
                        </div>
                    </div>
                        ...
                        <div class="insumo-card__item">
                            ...
                            <div class="insumo-card__value">
                                <input type="number" value="${cantidad}" min="1" style="width: 3.75rem;" 
                                       class="insumo-cantidad" data-precio="${
																					insumoInfo.precioNum
																				}" 
			...
			// Añadir evento al botón de eliminar
			const newCard = insumosGrid.querySelector(
				`.insumo-card[data-id="${insumoId}"]`
			);
			newCard
				.querySelector(".insumo-card__remove")
				.addEventListener("click", function () {
					newCard.remove();
					updateInsumoCounter();
				});
			...
			cantidadInput.addEventListener("input", function () {
				...
				document.getElementById(`costo-${insumoId}`).textContent =
	...
	function updateSensorCounter() {
		const sensorsGrid = document.querySelector(".tab-sensors .sensors-grid");
		const sensorHelper = document
			.querySelector("#sensorSelect")
			.closest(".form-field")
			.querySelector(".form-field__helper");
	...
	function updateInsumoCounter() {
		const insumosGrid = document.querySelector(".tab-insumos .sensors-grid");
		const insumoHelper = document
			.querySelector("#insumoSelect")
			.closest(".form-field")
			.querySelector(".form-field__helper");
	...
	function initDefaultInsumos() {
		...
		const defaultInsumos = [
			{
				id: "insumo2",
				name: "Fertilizante NPK Orgánico",
				type: "Fertilizante",
				color: "blue",
				proveedor: "EcoAgro",
				precio: "$650",
			...
			{
				id: "insumo5",
				name: "Semilla Premium Maíz Orgánico",
				type: "Semilla",
				color: "green",
				proveedor: "AgroSemillas S.A.",
				precio: "$1,200",
		...
		defaultInsumos.forEach((insumo) => {
			...
			const cardHtml = `
				...
				insumo.unidad
                        ...
                        <div class="insumo-card__item">
                            <span class="insumo-card__label">Costo Total:</span>
                            <span class="insumo-card__value insumo-costo-total" id="costo-${
															insumo.id
														}">$${costoTotal.toLocaleString()}</span>
                        </div>
                    </div>
                    <button class="insumo-card__remove" data-insumo-id="${
											insumo.id
										}">
                        <span class="material-symbols-outlined">delete</span>
                    </button>
