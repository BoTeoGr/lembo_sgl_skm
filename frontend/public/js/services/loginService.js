...
export async function loginUser(email, password) {
	try {
		...
		const userStatus =
			data.usuario.estado || data.usuario.status || data.usuario.estado_usuario;
		...
		try {
			if (data.usuario) {
				...
				const possibleId =
					u.id || u._id || u.usuario_id || u.userId || u.codigo;
				if (possibleId) {
			...
			if (data.usuario) {
				...
				if (u.tipo_documento || u.tipoDoc || u.tipoDocumento) {
					localStorage.setItem(
						"userTipoDoc",
						u.tipo_documento || u.tipoDoc || u.tipoDocumento
					);
				}
				if (u.numero_documento || u.numDoc || u.documento) {
					localStorage.setItem(
						"userNumDoc",
						u.numero_documento || u.numDoc || u.documento
					);
				}
			}
		} catch (_) {
