...
document.addEventListener("DOMContentLoaded", async () => {
	...
	try {
		...
		const response = await fetch(
			`http://localhost:5000/ciclo_cultivo/${cicloId}`,
			{
				headers: {
		...
		if (periodoInicio)
			periodoInicio.value = data.periodoInicio?.split("T")[0] || "";
		if (periodoFinal)
			periodoFinal.value = data.periodoFinal?.split("T")[0] || "";
		if (novedades) novedades.value = data.novedades || "";

		// Establecer el estado del radio button
		if (data.estado && estadoRadios.length > 0) {
			const estado =
				data.estado === "habilitado" ? "habilitado" : "deshabilitado";
			document.querySelector(
				`input[name="estado-habilitado"][value="${estado}"]`
			).checked = true;
...
if (form) {
	form.addEventListener("submit", async (e) => {
		...
		if (!cicloCultivo.nombre)
			return showToast(
				"Error",
				"El nombre del ciclo de cultivo es obligatorio",
				"error"
			);
		...
		if (!cicloCultivo.periodoInicio || !cicloCultivo.periodoFinal)
			return showToast(
				"Error",
				"Las fechas de inicio y final son obligatorias",
				"error"
			);
		if (
		...
		if (!cicloCultivo.novedades)
			return showToast(
				"Error",
				"Debe ingresar novedades del ciclo de cultivo",
				"error"
			);
		...
		if (!cicloId) {
			return showToast(
				"Error",
				"No se pudo obtener el ID del ciclo de cultivo",
				"error"
			);
		}
		...
		try {
			...
			if (!res.ok) {
				...
				throw new Error(
					errorData.message || "No se pudo actualizar el ciclo de cultivo"
				);
			}

			showToast(
				"Éxito",
				"Ciclo de cultivo actualizado correctamente",
				"success"
			);
			setTimeout(() => {
			...
			showToast(
				"Error",
				"Error al actualizar el ciclo de cultivo: " +
