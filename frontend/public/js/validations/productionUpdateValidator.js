...
document.addEventListener("DOMContentLoaded", async () => {
	...
	try {
		...
		form.startDate.value = data.fecha_de_inicio
		...
		form.estimatedProfit.value =
			data.meta_ganancia ||
			(data.inversion ? (parseFloat(data.inversion) * 1.3).toFixed(2) : "");
      ...
      <div class="item-info">
          ...
          <span class="item-details">Valor: $${
						supply.valor_unitario || ""
					}</span>
          <span class="item-details">Cantidad: ${
						supply.cantidad_usar || "Pendiente..."
					}</span>
      </div>
    `;
		...
		function renderSelectedSensors() {
			...
			selectedSensors.forEach((sensorId) => {
				...
				if (sensorSelect) {
					const option = Array.from(sensorSelect.options).find(
						(opt) => opt.value == sensorId
					);
    ...
    `;
		...
		showToast(
			"Error",
			"No se pudo cargar la producción: " + error.message,
			"error"
		);
	}
	...
	form.addEventListener("submit", async function (e) {
		...
		try {
			...
			showToast(
				"Error",
				error.message || "Error al actualizar la producción",
				"error"
			);
		}
	});
...
const closeCreateSensorModal = document.getElementById(
...
const closeCreateSupplyModal = document.getElementById(
...
const closeCreateCropCycleModal = document.getElementById(
	"closeCreateCropCycleModal"
);
...
async function getAllItems(endpoint, limit = 100) {
	try {
		...
		const response = await fetch(
			`${API_URL}${endpoint}?page=1&limit=${limit}`,
			{
				headers: headers,
			}
		);
		...
		if (!response.ok) {
			if (response.status === 401) {
				...
				window.location.href =
					"http://localhost:5501/frontend/public/views/login.html";
				throw new Error(
					"Sesión expirada o no autorizada. Redirigiendo al inicio de sesión..."
				);
			}
		...
		showToast(
			"Error",
			`No se pudieron cargar los datos de ${endpoint}`,
			"error"
		);
		return [];
	}
...
function updateAvailableSuppliesSelect() {
	...
	if (supplySelect.options.length <= 1) {
		...
		console.log(
			`Se agregaron ${
				supplySelect.options.length - 1
			} opciones al selector de insumos`
		);
	}
}
...
async function initializeForm() {
	try {
		...
		const [cropsData, cyclesData, sensorsData, suppliesData, usersData] =
			await Promise.all([
		...
		const crops = Array.isArray(cropsData)
			? cropsData
			: cropsData.cultivos || [];
		const cycles = Array.isArray(cyclesData)
			? cyclesData
			: cyclesData.ciclos || [];
		const sensors = Array.isArray(sensorsData)
			? sensorsData
			: sensorsData.sensores || [];
		const supplies = Array.isArray(suppliesData)
			? suppliesData
			: suppliesData.insumos || [];
		...
		const adminUsers = users.filter((user) => {
			...
			return (
				...
				userRole.includes("superadministrador")
			);
		});

		console.log(
			`Filtrando usuarios - Mostrando ${adminUsers.length} administradores/superadministradores de ${users.length} usuarios totales`
		);
		fillSelect(
			"responsible",
			adminUsers,
...
function fillSelect(elementId, items, defaultText, nameField, idField = "id") {
	...
	const enabledItems = items.filter(
		(item) => item && item.estado === "habilitado"
	);
	...
	enabledItems.forEach((item) => {
		...
		if (id && name) {
			...
			if (elementId == "crop" && item.cultivoId) {
				...
				console.log(
					`Item inválido en ${elementId} (id: ${id}, name: ${name}):`,
					item
				);
	...
	if (select.options.length <= 1) {
		...
		console.log(
			...
			} opciones al selector ${elementId}`
		);
	}
}
...
function validateInvestmentAndProfit() {
	...
	if (isNaN(investment) || investment <= 0) {
		if (investmentHelperText) {
			investmentHelperText.textContent =
				"La inversión debe ser un número mayor a 0";
		...
		if (investmentHelperText) {
			investmentHelperText.textContent =
				"Ingrese el monto total de inversión para esta producción";
	...
	if (isNaN(profit) || profit <= 0) {
		if (profitHelperText) {
			profitHelperText.textContent =
				"La meta de ganancias debe ser un número mayor a 0";
		...
		if (profitHelperText) {
			profitHelperText.textContent =
				"La meta de ganancias debe ser mayor o igual a la inversión";
...
function validateForm() {
	...
	const nombreProduccion = document
	...
	if (selectedSensors.size > 3) {
		...
		showToast(
			"Advertencia",
			"Se han seleccionado más de 3 sensores",
			"warning"
		);
	}
...
function calculateTotalInvestment() {
	...
	const selectedSuppliesFull = selectedSupplies
		.map((supply) => {
			const supplyData = allSuppliesGlobal.find(
				(s) => String(s.id) === String(supply.id)
			);
			return {
				...supplyData,
		...
		.filter(
			(supply) => supply && supply.valor_unitario && supply.cantidad_usar
		);
	...
	selectedSuppliesFull.forEach((supply) => {
		if (supply && supply.valor_unitario && supply.cantidad_usar) {
			totalInvestment +=
				Number.parseFloat(supply.valor_unitario) *
				Number.parseFloat(supply.cantidad_usar);
		}
	});
	...
	if (
		!estimatedProfitField.value ||
		Number.parseFloat(estimatedProfitField.value) < totalInvestment
	) {
...
function showSupplyUsageForm(supplyId) {
	...
	if (supplyUsageForm) {
      ...
      <div class="form-group">
        <label for="supplyUsageAmount">Cantidad a usar (${
					supply.nombre
				}):</label>
        <input type="number" class="form-control" id="supplyUsageAmount" min="0" step="0.01" value="${
					supply.cantidad_usar || 0
				}">
...
function handleSupplyUsage(supplyId) {
	...
	if (supplyIndex !== -1) {
		...
		console.log(
			`Actualizando insumo ${supplyId} con cantidad: ${cantidadUsar}`
		);
	}
...
function registerSupplyUsage() {
	...
	if (supplyIndex !== -1) {
		...
		const productionIndex = productionData.insumos_ids.findIndex(
			(s) => s.id === supply.id
		);
		if (productionIndex !== -1) {
			...
			productionData.insumos_ids[productionIndex] = {
				...
				cantidad_usar: quantity,
				esNuevo:
					prev && typeof prev.esNuevo !== "undefined" ? prev.esNuevo : true,
			};
		} else {
	...
	if (supplyCard) {
		const cantidadElement = supplyCard.querySelector(
...
function addSelectedSupply() {
	...
	const supply = allSuppliesGlobal.find(
		(s) => String(s.id) === String(supplyId)
	);
	if (!supply || !supply.valor_unitario || !supply.cantidad) {
		showToast(
			"Error",
			"Este insumo no tiene valor unitario o cantidad definidos",
			"error"
		);
		return;
	}
	...
	if (supplyUsageForm && supplyInfo && supplyUsageQuantity) {
		...
		document.getElementById(
			"unitValue"
		).textContent = `$${supply.valor_unitario}`;
	}
...
window.removeSelectedItem = (button, type) => {
	if (type === "sensor") {
		...
		if (productionData && Array.isArray(productionData.insumos_ids)) {
			const idx = productionData.insumos_ids.findIndex(
				(s) => String(s.id) === String(supplyId)
			);
			if (idx !== -1) {
		...
		selectedSupplies = selectedSupplies.filter(
			(s) => String(s.id) !== String(supplyId)
		);
		...
		if (
			insumoRemovido &&
			insumoRemovido.esNuevo === false &&
			insumoRemovido.cantidad_usar > 0
		) {
			fetch(`${API_URL}/insumos/reponer-stock`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					id: insumoRemovido.id,
					cantidad: insumoRemovido.cantidad_usar,
				}),
			})
				.then((res) => res.json())
				.then((data) => {
					if (data.success) {
						showToast(
							"Stock repuesto",
							"El stock del insumo fue repuesto correctamente",
							"success"
						);
					} else {
						showToast(
							"Atención",
							"No se pudo reponer el stock automáticamente",
							"error"
						);
					}
				})
		...
		suggestProfitBtn.addEventListener("click", (e) => {
			...
			if (totalInvestment && estimatedProfit) {
				...
				if (investment > 0) {
					...
					showToast(
						"Error",
						"Primero debe ingresar un valor válido para la inversión",
						"error"
					);
				}
			}
...
document
	.getElementById("hideSupplyUsageForm")
...
document
	.getElementById("modal-tipo-documento")
...
document
	.getElementById("modal-numero-documento")
...
document
	.getElementById("modal-confirmar-correo")
	.addEventListener("input", (e) => {
...
document
	.querySelectorAll('input[name="modal-estado-habilitado"]')
	.forEach((radio) => {
...
document
	.getElementById("modal-nombre-sensor")
	.addEventListener("keydown", (e) => {
		if (e.key >= "0" && e.key <= "9") {
			e.preventDefault();
			console.log("Número bloqueado");
...
document
	.getElementById("modal-unidad-medida")
	.addEventListener("change", (e) => {
...
document
	.getElementById("modal-tiempo-escaneo")
	.addEventListener("change", (e) => {
		modalSensorData.sensorScan = e.target.value;
	});

document
	.querySelectorAll('input[name="modal-estado-sensor"]')
	.forEach((radio) => {
		radio.addEventListener("change", (e) => {
...
// Bloquear números en el campo de nombre
document
	.getElementById("modal-nombre-insumo")
	.addEventListener("keydown", (e) => {
		if (e.key >= "0" && e.key <= "9") {
			e.preventDefault();
			console.log("Número bloqueado");
...
document
	.getElementById("modal-valor-unitario")
	.addEventListener("keydown", (e) => {
		if (
			e.key === "Backspace" ||
			e.key === "Tab" ||
...
document
	.getElementById("modal-medida-insumo")
	.addEventListener("change", (e) => {
...
document
	.getElementById("modal-descripcion-insumo")
	.addEventListener("input", (e) => {
...
document
	.querySelectorAll('input[name="modal-estado-insumo"]')
	.forEach((radio) => {
		radio.addEventListener("change", (e) => {
...
function validateModalSupplyData() {
	...
	if (
		isNaN(Number.parseFloat(modalSupplyData.insumePrice)) ||
		Number.parseFloat(modalSupplyData.insumePrice) <= 0
	) {
		showToast(
			"Error",
			"El valor unitario debe ser un número mayor a 0",
			"error"
		);
		return false;
	}
	...
	if (
		isNaN(Number.parseInt(modalSupplyData.insumeAmount)) ||
		Number.parseInt(modalSupplyData.insumeAmount) <= 0
	) {
...
createSupplyForm.addEventListener("submit", async (e) => {
	...
	modalSupplyData.insumeName = document.getElementById(
		"modal-nombre-insumo"
	).value;
	modalSupplyData.insumeType =
		document.getElementById("modal-tipo-insumo").value;
	modalSupplyData.insumeExtent = document.getElementById(
		"modal-medida-insumo"
	).value;
	modalSupplyData.insumePrice = document.getElementById(
		"modal-valor-unitario"
	).value;
	modalSupplyData.insumeAmount =
		document.getElementById("modal-cantidad").value;
	modalSupplyData.insumeDescription = document.getElementById(
		"modal-descripcion-insumo"
	).value;

	const estadoRadio = document.querySelector(
		'input[name="modal-estado-insumo"]:checked'
	);
	if (estadoRadio) {
...
// Bloquear números en el campo de nombre
document
	.getElementById("modal-nombre-cultivo")
	.addEventListener("keydown", (e) => {
		if (e.key >= "0" && e.key <= "9") {
			e.preventDefault();
			console.log("Número bloqueado");
...
document
	.getElementById("modal-ubicacion-cultivo")
	.addEventListener("input", (e) => {
		modalCropData.cultiveLocation = e.target.value;
	});

document
	.getElementById("modal-tamano-cultivo")
	.addEventListener("input", (e) => {
		modalCropData.cultiveSize = e.target.value;
	});

document
	.getElementById("modal-descripcion-cultivo")
	.addEventListener("input", (e) => {
		modalCropData.cultiveDescription = e.target.value;
	});

document
	.querySelectorAll('input[name="modal-estado-cultivo"]')
	.forEach((radio) => {
		radio.addEventListener("change", (e) => {
...
function validateModalCropData() {
	...
	if (
		isNaN(Number.parseFloat(modalCropData.cultiveSize)) ||
		Number.parseFloat(modalCropData.cultiveSize) <= 0
	) {
...
// Bloquear números en el campo de nombre
document
	.getElementById("modal-nombre-ciclo")
	.addEventListener("keydown", (e) => {
		if (e.key >= "0" && e.key <= "9") {
			e.preventDefault();
			console.log("Número bloqueado");
...
function validateModalCropCycleData() {
	...
	if (modalCropCycleData.estado === "deshabilitado") {
		showToast(
			"Error",
			"Cambia el estado para crear el ciclo de cultivo",
			"error"
		);
		return false;
	}
...
createCropCycleForm.addEventListener("submit", async (e) => {
	...
	try {
		...
		showToast(
			"Error",
			error.message || "No se pudo crear el ciclo de cultivo",
			"error"
		);
	}
});
...
function validarNombreProduccion(nombre) {
	if (!nombre) {
		return {
			valido: false,
			mensaje: "El nombre de la producción es requerido.",
		};
	}
	if (nombre.length < 3) {
		return {
			valido: false,
			mensaje: "El nombre de la producción debe tener al menos 3 caracteres.",
		};
	}
