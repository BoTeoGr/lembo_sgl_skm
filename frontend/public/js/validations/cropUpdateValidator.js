...
document.addEventListener("DOMContentLoaded", async () => {
	...
	form.addEventListener("submit", async (e) => {
		...
		if (!cultivoActual) {
			showToast(
				"Error",
				"No se puede actualizar sin datos del cultivo cargados.",
				"error"
			);
			return;
		}
		...
		if (
			...
			nombreInput.value.trim() !== cultivoActual.nombre
		) {
		...
		if (
			...
			tipoInput.value.trim() !== cultivoActual.tipo
		) {
		...
		if (
			...
			ubicacionInput.value.trim() !== cultivoActual.ubicacion
		) {
			datosActualizados.ubicacion = ubicacionInput.value.trim();
		}

		if (
			tamanoInput.value.trim() !== "" &&
			tamanoInput.value.trim() !== cultivoActual.tamano
		) {
		...
		if (
			...
			descripcionInput.value.trim() !== cultivoActual.descripcion
		) {
		...
		if (
			...
			imagenInput.value.trim() !== cultivoActual.imagen
		) {
		...
		try {
			...
			const errorMessage =
				error.message || "Hubo un error al actualizar el cultivo.";
