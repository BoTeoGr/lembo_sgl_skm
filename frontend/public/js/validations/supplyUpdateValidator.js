document.addEventListener("DOMContentLoaded", async () => {
	...
	const valorUnitarioInput = document.querySelector(
		".form__input--insume-price"
	);
	...
	form.addEventListener("submit", async (e) => {
		...
		if (
			...
			nombreInput.value.trim() !== insumoActual.nombre
		) {
		...
		if (
			...
			tipoInput.value.trim() !== insumoActual.tipo
		) {
			datosActualizados.tipo = tipoInput.value.trim();
		}

		if (
			medidaInput.value.trim() !== "" &&
			medidaInput.value.trim() !== insumoActual.unidad_medida
		) {
		...
		if (
			...
			descripcionInput.value.trim() !== insumoActual.descripcion
		) {
		...
		if (
			...
			imagenInput.value.trim() !== insumoActual.imagen
		) {
		...
		try {
			...
			const response = await fetch(
				`http://localhost:5000/insumos/${insumoId}`,
				{
					method: "PUT",
					headers: {
			...
			if (response.ok) {
				...
				showToast(
					"Error",
					data.error || "Error al actualizar el insumo",
					"error"
				);
			}
		} catch (error) {
