...
export function crearCultivo(req, res) {
	try {
		...
		if (!cultiveName) {
			return res
				.status(400)
				.json({ error: "El nombre del cultivo es obligatorio" });
		}

		if (!cultiveType) {
			return res
				.status(400)
				.json({ error: "El tipo de cultivo es obligatorio" });
		}

		if (!cultiveImage) {
			return res
				.status(400)
				.json({ error: "La imagen del cultivo es obligatoria" });
		}

		if (!cultiveLocation) {
			return res
				.status(400)
				.json({ error: "La ubicación del cultivo es obligatoria" });
		}

		if (!cultiveDescription) {
			return res
				.status(400)
				.json({ error: "La descripción del cultivo es obligatoria" });
		}
		...
		if (!cultiveSize) {
			return res
				.status(400)
				.json({ error: "El tamaño del cultivo es obligatorio" });
		}

		if (!estado) {
			return res
				.status(400)
				.json({ error: "El estado del cultivo es obligatorio" });
		}
		...
		if (isNaN(parsedSize) || parsedSize < 10 || parsedSize > 10000) {
			return res.status(400).json({
				error:
					"El tamaño del cultivo debe ser un número válido entre 10 y 10000 m²",
			});
		}
		...
		if (estado === "deshabilitado") {
			return res
				.status(400)
				.json({
					error: "No se puede crear un sensor con el estado 'deshabilitado'",
				});
		}
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
					...
					(err, results) => {
						if (err) {
							...
							return res
								.status(500)
								.json({ error: "Error desconocido al crear el cultivo" });
						}
						res.status(201).json({
...
export function actualizarEstadoCultivo(req, res) {
	...
	const estadoDB =
		estado === "Activo" || estado.toLowerCase() === "habilitado"
			? "habilitado"
			: "deshabilitado";
	...
	db.query(query, [estadoDB, id], (err, result) => {
		if (err) {
			...
			return res
				.status(500)
				.json({
					error: "Error al actualizar el estado del cultivo",
					detalle: err.message,
				});
		}
...
export function actualizarCultivo(req, res) {
	const { id } = req.params;
	const { nombre, tipo, ubicacion, tamano, descripcion, estado } = req.body;
