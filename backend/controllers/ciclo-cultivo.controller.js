	...
	try {
		...
		db.query(query, (err, results) => {
			if (err) {
				...
				return res
					.status(500)
					.json({ error: "Error al obtener ciclos de cultivo" });
			}
...
export function crearCicloCultivo(req, res) {
	try {
		...
		if (!dateRegex.test(cycleStartDate) || !dateRegex.test(cycleEndDate)) {
			return res
				.status(400)
				.json({ error: "Formato de fecha no válido. Use YYYY-MM-DD" });
		}
		...
		db.query(
			...
			(err, results) => {
				if (err) {
					...
					return res
						.status(500)
						.json({ error: "Error desconocido al crear el ciclo de cultivo" });
				}
				res
					.status(201)
					.json({
						message: "Ciclo de cultivo creado correctamente",
						cicloCultivoId: results.insertId,
					});
			}
		);
	} catch (error) {
...
export function obtenerCicloCultivoPorId(req, res) {
	...
	db.query("SELECT * FROM ciclo_cultivo WHERE id = ?", [id], (err, results) => {
		if (err) {
			...
			return res
				.status(500)
				.json({ error: "Error al obtener ciclo de cultivo" });
		}
		if (results.length === 0) {
...
export function actualizarCicloCultivo(req, res) {
	...
	db.query(
		query,
		[nombre, descripcion, periodoInicio, periodoFinal, novedades, estado, id],
		(err, result) => {
			if (err) {
				console.error("Error al actualizar ciclo de cultivo:", err);
				return res
					.status(500)
					.json({ error: "Error al actualizar ciclo de cultivo." });
			}
			if (result.affectedRows === 0) {
				return res
					.status(404)
					.json({ error: "Ciclo de cultivo no encontrado." });
			}
			res
				.status(200)
				.json({ message: "Ciclo de cultivo actualizado correctamente." });
		}
	);
}
	...
	try {
		...
		db.query(query, [estado, id], (err, result) => {
			if (err) {
				...
				return res
					.status(500)
					.json({ error: "Error al actualizar estado del ciclo de cultivo" });
			}
