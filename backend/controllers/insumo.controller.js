...
export function VerInsumos(req, res) {
	db.query(
		"SELECT id, nombre, tipo, imagen, unidad_medida, valor_unitario, cantidad, valor_total, descripcion, usuario_id, estado, fecha_creacion FROM insumos",
		(err, results) => {
			if (err) {
				console.error("Error al obtener insumos:", err);
				return res
					.status(500)
					.json({ error: "Error al obtener insumos", details: err.message });
			}
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
					...
					(err, results) => {
						if (err) {
							...
							return res
								.status(500)
								.json({ error: "Error desconocido al crear el insumo" });
						}
						res
							.status(201)
							.json({
								message: "Insumo creado correctamente",
								insumoId: results.insertId,
							});
					}
				);
			}
	...
	db.query(query, [estadoDB, id], (err, result) => {
		if (err) {
			...
			return res
				.status(500)
				.json({
					error: "Error al actualizar el estado del insumo",
					detalle: err.message,
				});
		}
...
export function obtenerResumenInsumos(req, res) {
	...
	db.query(query, (err, results) => {
		if (err) {
			...
			return res
				.status(500)
				.json({ error: "Error al obtener el resumen de insumos" });
		}
		...
		const resumenArray = Object.entries(resumen).map(([tipo, datos]) => ({
			...
			porcentaje:
				totalInsumos > 0
					? Math.round((datos.cantidad / totalInsumos) * 100)
					: 0,
		}));
